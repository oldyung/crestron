...
const page1Module = (() => {
    ...
    function handleNavClick(ev) {
        ...
        if (!a) return;
        if (a === 'power') { pulseDigital(2); }
        else if (a === 'home') { pulseDigital(75); }
        else if (a === 'mic') { pulseDigital(70); }
        else if (a === 'volume') { pulseDigital(71); }
        else if (a === 'blinds') { pulseDigital(72); }
        else if (a === 'lighting') { pulseDigital(73); }
        else if (a === 'equipment') { pulseDigital(74); }
    }
    ...
    function systemOffConfirm(confirmed) {
        if (confirmed) pulseDigital(57);
        else pulseDigital(58);
    }
    ...
    function onInit() {
        ...
        var overlayMap = ['mic', 'volume', 'blinds', 'lighting', 'equipment'];
        [50, 51, 52, 53, 54].forEach(function (jn, i) {
            CrComLib.subscribeState('b', jn.toString(), function (v) {
                if (v) showOverlay('overlay-' + overlayMap[jn - 50]);
                else closeOverlay('overlay-' + overlayMap[jn - 50]);
            });
        });
        CrComLib.subscribeState('b', '55', function (v) {
            if (v) closeAllOverlays();
        });
        CrComLib.subscribeState('b', '56', function (v) {
            if (v) navigateTo('page-system-off');
        });
        CrComLib.subscribeState('b', '20', function (v) {
            if (v) navigateTo('page-logo');
        });
        CrComLib.subscribeState('b', '21', function (v) {
            if (v) navigateTo('page-home');
        });
        CrComLib.subscribeState('b', '22', function (v) {
            if (v) navigateTo('page-presentation');
        });
        CrComLib.subscribeState('b', '23', function (v) {
            if (v) navigateTo('page-vc');
        });
    }
